...
def create_global_env():
    ...
    env.update({
        ...
        'is-alpha?': lambda s: s.isalpha() if s else False,
        'is-digit?': lambda s: s.isdigit() if s else False,
        'hash256': lambda s: __import__('hashlib').sha256(str(s).encode('utf-8')).hexdigest(),
        'read-file': lambda p: open(p, "r", encoding="utf-8").read(),
        'write-file': lambda p, d: open(p, "w", encoding="utf-8").write(str(d)),
